...
import { getPostBySlug, getPageContent, generateContentFromExcerpt } from "@/lib/notion"
...
export async function generateStaticParams() {
  try {
    const posts = await getPostBySlug() // Assuming getPostBySlug can fetch all posts
...
async function BlogPostContent({ slug }: { slug: string }) {
  try {
    ...
    if (!displayContent || displayContent.trim() === "") {
      displayContent = generateContentFromExcerpt(post.excerpt, post.title)
    }
...
export default async function BlogPostPage({ params }: { params: { slug: string } }) {
  ...
  try {
    console.log(`Attempting to fetch content for post: ${post.id}`)
    content = await getPageContent(post.id)
    console.log(`Content fetch complete, length: ${content.length}`)
  } catch (error) {
    console.error(`Error fetching content for post ${post.id}:`, error)
    contentError = true
    // Generate fallback content from the excerpt
    content = generateContentFromExcerpt(post.excerpt, post.title)
  }
    ...
    <div className="min-h-screen bg-[#1E1E2A] text-white">
      <div className="container mx-auto px-4 py-12">
        ...
        <Suspense fallback={<LoadingFallback />}>
          <BlogPostClient post={post} content={content} contentError={contentError} />
        </Suspense>
      </div>
    </div>
  )
